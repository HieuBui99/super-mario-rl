...
while step < train_steps:
    ...
    for t in range(max_episode_len):
        ...
        if step % update_steps == 0:
            ...
            rewards = []
            gae = 0
            for reward, value, is_terminal in list(zip(replay_buffer.rewards, replay_buffer.state_values, replay_buffer.is_terminals))[::-1]:
                gae = gae * gamma * tau
                gae = gae + reward + gamma * next_value.detach() * (1 - is_terminal) - value.detach()
                next_value = value
            ...
            for _ in range(epochs):
                ...
                for j in range(0, len(indices), bs):
                    ...
                    logprobs, state_values, dist_entropy = model.evaluate(old_obs[batch_indices], old_actions[batch_indices])
                    
                    ratio = torch.exp(logprobs - old_logprobs[batch_indices])
                    surr1 = ratio * advantages[batch_indices]
                    surr2 = torch.clamp(ratio, 1-eps, 1+eps) * advantages[batch_indices]
                    actor_loss = -torch.min(surr1, surr2).mean()

                    critic_loss = 0.5 * F.mse_loss(state_values.squeeze(-1), rewards[batch_indices])
        ...
        if step % print_freq == 0:
            print(f"Step: {step}, Episode: {episode}, Reward: {running_reward / running_episodes}")
            running_reward = 0
            running_episodes = 0
